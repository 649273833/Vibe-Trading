...
- **2026-08-18** 🈶 **一份正确的报告，因为日期紧挨着中文而被拒答**：grounding 闸门会先屏蔽日历日期，再把剩下的数字当作价格声明去核对，但它的 ISO 日期模式用 `\b` 定界，而 `\b` 是 Unicode 感知的——`最` 也算词字符，于是 `(2026-07-14最低)` 在“日”之后根本没有边界。日期逃过屏蔽，把 `2026`、`7`、`14` 贡献成三个候选价格，它们与任何已观测的 OHLC 区间都对不上，于是一份报价本身完全正确的报告被拒绝（[#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132)，关闭 [#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122)）。同批：`run_dcf` 会拿非有限输入照常估值而不是拒绝——`beta=inf` 时返回每股 `-1.7`，因为 `math.isclose(inf, inf)` 为 `True`，对账网从未触发（[#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121)，关闭 [#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120)）；某个工具模块导入失败，只会留下一个悄悄变小的注册表和一句笼统的 `Tool not found`，缺失的可选依赖看起来就像拼错了工具名（[#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129)，关闭 [#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124)）；MCP 的 `get_market_data` docstring 让客户端去读 `_provenance.volume_unit`，而调用处没传 `include_provenance=True`，这个字段从来就不存在（[#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)）。**新增：** **德语成为第 7 种界面语言**，并且 locale 一致性测试改为从 locales 目录派生语言集合，新语言不可能再零覆盖上线（[#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117)）；另有离线 USD-M 账户对账，在不建立任何连接的前提下，把本地风险状态与一次交易所观测做比对（[#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)）。感谢 [@Robin1987China](https://github.com/Robin1987China)、[@newgo](https://github.com/newgo)、[@er-s-an](https://github.com/er-s-an)、[@Shizoqua](https://github.com/Shizoqua)、[@1psconstructor](https://github.com/1psconstructor) 和 [@honginp](https://github.com/honginp)！
