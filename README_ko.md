...
- **2026-08-18** 🈶 **날짜가 중국어에 붙어 있었다는 이유로 올바른 리포트가 거부됐습니다**: grounding 게이트는 남은 숫자를 가격 주장으로 읽기 전에 달력 날짜를 마스킹하지만, ISO 날짜 패턴이 `\b`에 고정돼 있었습니다. `\b`는 유니코드를 인식하므로 `最`도 단어 문자로 취급되고, 따라서 `(2026-07-14最低)`에는 '일' 뒤에 경계가 없습니다. 날짜가 마스킹을 빠져나가 `2026`, `7`, `14`를 후보 가격으로 넘겼고, 이 값들은 관측된 어떤 OHLC 범위와도 맞지 않아 인용값 자체는 정확한 리포트가 거부됐습니다([#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132), [#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122) 종료). 함께: `run_dcf`는 비유한 입력을 거부하지 않고 그대로 평가해 `beta=inf`에서 주당 `-1.7`을 반환했습니다. `math.isclose(inf, inf)`가 `True`라 대조 안전망이 한 번도 작동하지 않았기 때문입니다([#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121), [#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120) 종료). 임포트에 실패한 도구 모듈은 조용히 줄어든 레지스트리와 일반적인 `Tool not found`만 남겨서, 누락된 선택적 의존성이 오타처럼 보였습니다([#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129), [#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124) 종료). MCP의 `get_market_data`는 docstring에서 `_provenance.volume_unit`을 읽으라고 안내하면서 호출부는 `include_provenance=True`를 넘기지 않아 그 필드가 아예 없었습니다([#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)). **새 기능:** **독일어가 7번째 UI 언어**가 됐고, 로케일 정합성 스위트가 언어 집합을 locales 디렉터리에서 도출하도록 바뀌어 새 로케일이 테스트 없이 출시될 수 없습니다([#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117)). 여기에 연결을 전혀 열지 않고 로컬 리스크 상태와 거래소 관측치를 비교하는 오프라인 USD-M 계정 대사가 추가됐습니다([#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)). [@Robin1987China](https://github.com/Robin1987China), [@newgo](https://github.com/newgo), [@er-s-an](https://github.com/er-s-an), [@Shizoqua](https://github.com/Shizoqua), [@1psconstructor](https://github.com/1psconstructor), [@honginp](https://github.com/honginp) 님 감사합니다!
