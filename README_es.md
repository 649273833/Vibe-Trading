...
- **2026-08-18** 🈶 **Un informe correcto se rechazaba porque una fecha tocaba texto en chino**: la puerta de grounding enmascara las fechas de calendario antes de leer el resto de los números como afirmaciones de precio, pero su patrón ISO estaba anclado en `\b`, que reconoce Unicode: `最` cuenta como carácter de palabra, así que en `(2026-07-14最低)` no hay frontera después del día. La fecha sobrevivía al enmascarado y aportaba `2026`, `7` y `14` como precios candidatos, ninguno de los cuales coincide con un rango OHLC observado, de modo que se rechazaba un informe cuyas cotizaciones reales eran correctas ([#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132), cierra [#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122)). Además: `run_dcf` valoraba entradas no finitas en lugar de rechazarlas — con `beta=inf` devolvía un precio por acción de `-1.7`, porque `math.isclose(inf, inf)` es `True` y la red de conciliación nunca se activaba ([#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121), cierra [#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120)); un módulo de herramientas que fallaba al importarse dejaba un registro silenciosamente más pequeño tras un `Tool not found` genérico, así que una dependencia opcional ausente parecía un error de escritura ([#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129), cierra [#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124)); y el docstring de `get_market_data` en MCP indicaba a los clientes leer `_provenance.volume_unit` mientras la llamada omitía `include_provenance=True`, así que el campo nunca estuvo ahí ([#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)). **Novedades:** **el alemán es el séptimo idioma de la interfaz**, y la suite de paridad de locales deriva ahora su conjunto de idiomas del directorio locales, de modo que un idioma nuevo no puede publicarse sin pruebas ([#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117)); además, conciliación de cuenta USD-M sin conexión, que compara el estado de riesgo local con una observación del exchange sin abrir ninguna conexión ([#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)). Gracias [@Robin1987China](https://github.com/Robin1987China), [@newgo](https://github.com/newgo), [@er-s-an](https://github.com/er-s-an), [@Shizoqua](https://github.com/Shizoqua), [@1psconstructor](https://github.com/1psconstructor) y [@honginp](https://github.com/honginp).
