...
- **2026-08-22** 🧮 **Los retornos de factores dejaron de inventar precios a través de huecos**: Trece factores del Alpha Zoo calculaban su serie de retornos con un `pct_change()` desnudo, que rellena hacia delante el cierre faltante antes de diferenciar — una exclusión de cotización, un festivo no compartido o una caída del proveedor se convertían en un «retorno del 0 %» finito que el ranking trataba como información. El hueco ahora se mantiene `NaN`, la política que el resto del zoo ya seguía ([#1172](https://github.com/HKUDS/Vibe-Trading/pull/1172)). **Corregido:** clientes MCP independientes en un mismo servidor http/sse compartían una única sesión de objetivo de investigación de reserva, de modo que el `start_research_goal` de uno podía reemplazar el de otro — los objetivos ahora se distinguen por el id de sesión por conexión de FastMCP ([#1173](https://github.com/HKUDS/Vibe-Trading/pull/1173)); la recolección de basura y la compresión de memoria dejaban filas FTS obsoletas y sidecars de relaciones huérfanos ([#1174](https://github.com/HKUDS/Vibe-Trading/pull/1174)); `cancel_run()` nunca llegaba a un worker swarm que ya estaba transmitiendo, que seguía despachando llamadas a herramientas después de reportarse la ejecución como cancelada — la parada ahora interrumpe el flujo, omite las llamadas de ese turno y queda como tarea *cancelada* en lugar de fallida ([#1175](https://github.com/HKUDS/Vibe-Trading/pull/1175)); `get_research_reports` por MCP descartaba `beginTime`/`endTime` a la entrada ([#1176](https://github.com/HKUDS/Vibe-Trading/pull/1176)); y `get_options_chain` respondía a un vencimiento de otro ciclo con `ok: true` y los contratos de otra fecha — un vencimiento explícito ahora se valida contra la cadena devuelta ([#1177](https://github.com/HKUDS/Vibe-Trading/pull/1177)). **Nuevo:** una página **Cartera** de solo lectura agrega las posiciones de tus conexiones de broker de solo lectura — procedencia por fuente, instantáneas inmutables, fuentes fallidas excluidas en lugar de arrastradas, exportación CSV y una herramienta `portfolio_summary` que alimenta a `portfolio_risk_xray` ([#1072](https://github.com/HKUDS/Vibe-Trading/pull/1072)). ¡Gracias [@Shizoqua](https://github.com/Shizoqua), [@cgycorey](https://github.com/cgycorey) y [@goatyyc](https://github.com/goatyyc)!
