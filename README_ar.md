...
- **2026-08-18** 🈶 **تقرير صحيح كان يُرفض لأن تاريخًا لامس نصًا صينيًا**: تُخفي بوابة grounding تواريخ التقويم قبل أن تقرأ بقية الأرقام بوصفها ادعاءات سعرية، لكن نمط التاريخ ISO كان مثبّتًا على `\b`، و`\b` يراعي Unicode، فيُحتسب `最` حرفًا ضمن الكلمة، ومن ثم لا توجد حدود بعد اليوم في `(2026-07-14最低)`. نجا التاريخ من الإخفاء وأسهم بـ `2026` و`7` و`14` كأسعار مرشّحة لا يطابق أيٌّ منها نطاق OHLC مرصودًا، فرُفض تقرير كانت اقتباساته الفعلية صحيحة ([#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132)، يغلق [#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122)). وأيضًا: كان `run_dcf` يُقيّم المدخلات غير المنتهية بدل رفضها — فمع `beta=inf` كان يعيد سعرًا للسهم قدره `-1.7`، لأن `math.isclose(inf, inf)` تساوي `True` فلم تعمل شبكة المطابقة قط ([#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121)، يغلق [#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120))؛ وأي وحدة أدوات يفشل استيرادها كانت تترك سجلًا أصغر بصمت خلف رسالة `Tool not found` عامة، فتبدو التبعية الاختيارية المفقودة وكأنها خطأ إملائي ([#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129)، يغلق [#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124))؛ كما كان توثيق `get_market_data` في MCP يطلب من العملاء قراءة `_provenance.volume_unit` بينما لم يمرّر الاستدعاء `include_provenance=True`، فلم يكن الحقل موجودًا أصلًا ([#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)). **جديد:** **الألمانية هي لغة الواجهة السابعة**، وصارت مجموعة اختبارات تطابق اللغات تشتق قائمتها من مجلد locales، فلا يمكن إطلاق لغة جديدة بلا اختبار ([#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117))؛ إضافة إلى تسوية حساب USD-M دون اتصال، تقارن حالة المخاطر المحلية برصدٍ من المنصة دون فتح أي اتصال ([#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)). شكرًا [@Robin1987China](https://github.com/Robin1987China) و[@newgo](https://github.com/newgo) و[@er-s-an](https://github.com/er-s-an) و[@Shizoqua](https://github.com/Shizoqua) و[@1psconstructor](https://github.com/1psconstructor) و[@honginp](https://github.com/honginp)!
