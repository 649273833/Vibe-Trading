...
- **2026-08-22** 🧮 **توقّفت عوائد العوامل عن اختلاق أسعار عبر الفجوات**: كانت 13 من عوامل Alpha Zoo تحسب سلسلة عوائدها بـ `pct_change()` مجرّدة، وهي تملأ الإغلاق المفقود للأمام قبل الفرق — فيتحوّل الشطب أو عطلة غير مشتركة أو انقطاع المزوّد إلى «عائد 0%» محدود يعامله الترتيب كمعلومة. تبقى الفجوة الآن `NaN`، وهي السياسة التي يتبعها بقية الـ zoo أصلًا ([#1172](https://github.com/HKUDS/Vibe-Trading/pull/1172)). **أُصلح:** كان عملاء MCP المستقلّون على خادم http/sse واحد يتشاركون جلسة هدف بحثي احتياطية واحدة، فيستبدل `start_research_goal` لأحدهم هدف الآخر — تُميَّز الأهداف الآن بمعرّف جلسة FastMCP لكل اتصال ([#1173](https://github.com/HKUDS/Vibe-Trading/pull/1173))؛ وكان جمع قمامة الذاكرة والضغط يتركان صفوف FTS قديمة وملفات علاقات يتيمة ([#1174](https://github.com/HKUDS/Vibe-Trading/pull/1174))؛ ولم يكن `cancel_run()` يصل إلى عامل swarm يبثّ بالفعل، فيواصل إرسال استدعاءات الأدوات بعد الإبلاغ عن إلغاء التشغيل — يقطع الإيقاف الآن البثّ ويتخطّى استدعاءات تلك الجولة ويُسجَّل كمهمة *ملغاة* لا فاشلة ([#1175](https://github.com/HKUDS/Vibe-Trading/pull/1175))؛ وكان `get_research_reports` عبر MCP يُسقط `beginTime`/`endTime` عند المدخل ([#1176](https://github.com/HKUDS/Vibe-Trading/pull/1176))؛ وكان `get_options_chain` يجيب عن تاريخ انتهاء من دورة أخرى بـ `ok: true` وعقود تاريخ آخر — يُتحقَّق الآن من تاريخ الانتهاء الصريح مقابل السلسلة المُعادة ([#1177](https://github.com/HKUDS/Vibe-Trading/pull/1177)). **جديد:** صفحة **المحفظة** للقراءة فقط تجمّع مراكزك عبر اتصالات الوسطاء للقراءة فقط — مصدر كل مركز، ولقطات غير قابلة للتعديل، واستبعاد المصادر الفاشلة بدل ترحيلها، وتصدير CSV، وأداة `portfolio_summary` التي تغذّي `portfolio_risk_xray` ([#1072](https://github.com/HKUDS/Vibe-Trading/pull/1072)). شكرًا [@Shizoqua](https://github.com/Shizoqua)، [@cgycorey](https://github.com/cgycorey)، [@goatyyc](https://github.com/goatyyc)!
