...
class _RedirectCompatibleOAuth(OAuth):
    """FastMCP OAuth with support for standard HTTP 301 authorization redirects."""
    ...
    def _pin_ibkr_oauth_metadata(self) -> None:
        """Pin IBKR metadata before token refresh as well as initial consent."""
        if (urlsplit(self.mcp_url).hostname or "").lower() != "api.ibkr.com":
            return
        self.context.auth_server_url = "https://api.ibkr.com/oauth2"
    ...
    async def _initialize(self) -> None:
        ...
        self._pin_ibkr_oauth_metadata()

    async def async_auth_flow(self, request: httpx.Request):
        """Add WAF-compatible headers to every request generated by OAuth."""
        flow = super().async_auth_flow(request)
        try:
            outgoing = await flow.__anext__()
            while True:
                if (outgoing.url.host or "").lower() == "api.ibkr.com":
                    outgoing.headers["User-Agent"] = self._IBKR_USER_AGENT
                    outgoing.headers.setdefault("Accept", "application/json, text/html;q=0.9, */*;q=0.8")
    ...
    async def redirect_handler(self, authorization_url: str) -> None:
        if not self._allow_interactive:
            raise RuntimeError("OAuth reauthorization required; use the explicit reconnect action")
        parsed = urlsplit(authorization_url)
        if (parsed.hostname or "").lower() == "api.ibkr.com":
            requested_scopes = (
                list(self._scopes)
                if isinstance(self._scopes, list)
                else str(self._scopes or "").split()
            )
            if requested_scopes != ["mcp.read"]:
                raise RuntimeError("IBKR OAuth must request exactly the mcp.read scope")
            ...
            # IBKR's protected-resource discovery currently causes some MCP
            # clients to fall back to root /authorize and /token endpoints.
            # Pin the server's published OAuth2 metadata after discovery so the
            # subsequent code exchange also uses the official token endpoint.
            self._pin_ibkr_oauth_metadata()
        async with self.httpx_client_factory() as client:
            response = await client.get(authorization_url, follow_redirects=False)
            if response.status_code == 400:
                raise RuntimeError("OAuth client not found; re-register the client")
            if response.status_code not in (200, 301, 302, 303, 307, 308):
                raise RuntimeError(
                    f"Unexpected authorization response: {response.status_code}"
                )
        logger.info("OAuth authorization URL: %s", authorization_url)
