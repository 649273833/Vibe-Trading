...
- **2026-08-18** 🈶 **A correct report was refused because a date touched Chinese text**: The grounding gate masks calendar dates before reading the remaining numbers as price claims, but its ISO pattern was anchored on `\b` — which is Unicode-aware, so `最` counts as a word character and `(2026-07-14最低)` has no boundary after the day. The date survived the mask and contributed `2026`, `7` and `14` as candidate prices, none of which match any observed OHLC range, so a report whose actual quotes were correct was rejected ([#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132), closes [#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122)). Also: `run_dcf` valued non-finite inputs instead of refusing them — `beta=inf` returned a per-share price of `-1.7`, because `math.isclose(inf, inf)` is `True` and the reconciliation net never fired ([#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121), closes [#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120)); a tool module that failed to import left a quietly smaller registry behind a generic `Tool not found`, so a missing optional dependency read as a typo ([#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129), closes [#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124)); and the MCP `get_market_data` docstring told clients to read `_provenance.volume_unit` while the call omitted `include_provenance=True`, so the field was never there ([#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)). **New:** **German is the seventh UI language**, and the locale parity suite now derives its language set from the locales directory, so a new locale cannot ship untested ([#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117)); plus offline USD-M account reconciliation, comparing local risk state with an exchange observation without opening a connection ([#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)). Thanks [@Robin1987China](https://github.com/Robin1987China), [@newgo](https://github.com/newgo), [@er-s-an](https://github.com/er-s-an), [@Shizoqua](https://github.com/Shizoqua), [@1psconstructor](https://github.com/1psconstructor), and [@honginp](https://github.com/honginp)!
