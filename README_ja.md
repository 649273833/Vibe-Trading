...
- **2026-08-18** 🈶 **日付が中国語に接していたせいで、正しいレポートが拒否されていました**：grounding ゲートは残りの数値を価格の主張として読む前にカレンダー日付をマスクしますが、その ISO 日付パターンは `\b` を境界に使っていました。`\b` は Unicode を認識するため `最` も語構成文字と見なされ、`(2026-07-14最低)` では「日」の直後に境界が存在しません。日付はマスクをすり抜けて `2026`・`7`・`14` を候補価格として持ち込み、いずれも観測済みの OHLC レンジと一致しないため、引用値そのものは正しいレポートが拒否されていました（[#1132](https://github.com/HKUDS/Vibe-Trading/pull/1132)、[#1122](https://github.com/HKUDS/Vibe-Trading/issues/1122) をクローズ）。同時に：`run_dcf` は非有限の入力を拒否せずそのまま評価していました。`beta=inf` で 1 株あたり `-1.7` を返していたのは、`math.isclose(inf, inf)` が `True` になり突合ネットが一度も作動しなかったためです（[#1121](https://github.com/HKUDS/Vibe-Trading/pull/1121)、[#1120](https://github.com/HKUDS/Vibe-Trading/issues/1120) をクローズ）。インポートに失敗したツールモジュールは、静かに小さくなったレジストリと一般的な `Tool not found` だけを残すため、欠けているオプション依存がタイプミスのように見えていました（[#1129](https://github.com/HKUDS/Vibe-Trading/pull/1129)、[#1124](https://github.com/HKUDS/Vibe-Trading/issues/1124) をクローズ）。MCP の `get_market_data` は docstring で `_provenance.volume_unit` を読むよう指示しながら、呼び出し側が `include_provenance=True` を渡しておらず、このフィールドは存在していませんでした（[#1131](https://github.com/HKUDS/Vibe-Trading/pull/1131)）。**新機能：** **ドイツ語が 7 番目の UI 言語**になり、ロケール整合性スイートは言語一覧を locales ディレクトリから導出するようになったため、新しいロケールが未テストのまま出荷されることはありません（[#1117](https://github.com/HKUDS/Vibe-Trading/pull/1117)）。さらに、接続を一切開かずにローカルのリスク状態と取引所の観測値を比較するオフライン USD-M 口座リコンサイル（[#1106](https://github.com/HKUDS/Vibe-Trading/pull/1106)）。[@Robin1987China](https://github.com/Robin1987China)、[@newgo](https://github.com/newgo)、[@er-s-an](https://github.com/er-s-an)、[@Shizoqua](https://github.com/Shizoqua)、[@1psconstructor](https://github.com/1psconstructor)、[@honginp](https://github.com/honginp) に感謝します！
